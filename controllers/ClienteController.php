...
class ClienteController
{
    public function CargarUno(Request $request, Response $response, array $args)
    {
        ...
        if (empty($data['id_comanda']) || empty($data['id_encuesta']) || empty($data['nombre']) || empty($data['hora_llegada']) || empty($data['estado'])) {
        ...
        $nuevoCliente->id_encuesta = $data['id_encuesta'];
    ...
    public function ModificarUno(Request $request, Response $response, array $args)
    {
        ...
        if (empty($data['idCliente']) || empty($data['id_comanda']) || empty($data['id_encuesta']) || empty($data['nombre']) || empty($data['hora_llegada']) || empty($data['estado'])) {
            $response->getBody()->write(json_encode(array('error' => 'Todos los campos son obligatorios')));
            return $response->withStatus(400)->withHeader('Content-Type', 'application/json');
        }
        ...
        $cliente->id_encuesta = $data['id_encuesta'];
